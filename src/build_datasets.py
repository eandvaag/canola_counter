...
def build_inference_datasets(config):
    ...
    model_dir = os.path.join("usr", "data", "models", config["model_uuid"])
    
    for image_set_config in config["inference"]["image_sets"]:
        farm_name = image_set_config["farm_name"]
        field_name = image_set_config["field_name"]
        mission_date = image_set_config["mission_date"]
        ...
        patch_dir = os.path.join(model_dir, "inference_patches", 
                                 farm_name + "_" + field_name + "_" + mission_date)
        ...
        for image in tqdm.tqdm(images, desc="Extracting inference patches"):
...
def build_direct_tiled(config):
    ...
    for image_set_config in config["training"]["image_sets"]:
        ...
        if len(completed_images) > 0 and num_annotations > 30:
            ...
            for image_name in completed_images:
                if image_name not in test_reserved_images:
    ...
    usr_data_root = os.path.join("usr", "data")
    patches_dir = os.path.join(usr_data_root, "models", config["model_uuid"], "training_patches", "0")
    training_patches_dir = os.path.join(patches_dir, "training")
    validation_patches_dir = os.path.join(patches_dir, "validation")
    os.makedirs(training_patches_dir)
    os.makedirs(validation_patches_dir)
